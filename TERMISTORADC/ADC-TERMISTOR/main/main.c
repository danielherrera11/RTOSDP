...
esp_err_t create_task(void)
{
    ...
    xTaskCreate(vTaskadc,
                "READ_ADC", /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
                STACK_SIZE,
                &ucParameterToPass,
                1,
                &xHandle);
...
// crear funcion tarea adc
void vTaskadc(void *pvParameters)
{
    while (1)
    {
        adc_val = adc1_get_raw(ADC1_CHANNEL_6);
        printf("el valor medido por el adc es: %i\r\n", adc_val);
